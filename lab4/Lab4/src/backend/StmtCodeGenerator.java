...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		...
		WhileStmt whileStatement = nd.getEnclosingLoop();
		Unit breakStatementTargetUnit = this.breakTargets.get(whileStatement);
		this.units.add(j.newGotoStmt(breakStatementTargetUnit));
		return null; 
	}
	...
	public Void visitWhileStmt(WhileStmt nd) {
		...
		this.units.add(
				j.newIfStmt(
						j.newEqExpr(condition, IntConstant.v(0)),
						label1
						)
					);
		this.breakTargets.put(nd, label1); // break 
