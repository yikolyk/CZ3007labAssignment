...
public class ExprCodeGenerator extends Visitor<Value> {
	...
	public Value visitIntLiteral(IntLiteral nd) {
		...
		//only add statement rather than expression 
		return value;
	}
	...
	public Value visitBooleanLiteral(BooleanLiteral nd) {
		...
		Boolean booleanValue = nd.getValue();
		Value intValue;
		
		//no need wrap for boolean value
		if(booleanValue) {
			intValue = IntConstant.v(1); 
		}
		else {
			intValue = IntConstant.v(0); 
		}
		return intValue;
	}
	...
	public Value visitArrayIndex(ArrayIndex nd) {
		...
		Value arrayIndex = this.wrap(nd.getIndex().accept(this));
		
		// need to wrap int [][] a;
		Value arrayBase = this.wrap(nd.getBase().accept(this)); 
		return Jimple.v().newArrayRef(arrayBase, arrayIndex);
	}
	...
	public Value visitBinaryExpr(BinaryExpr nd) {
		 ...
		 */
		final Value leftHandSideValue = this.wrap(nd.getLeft().accept(this));
		final Value rightHandSideValue = this.wrap(nd.getRight().accept(this));
		Value binValue = this.wrap(nd.accept(new Visitor<Value>() {
			@Override
			public Value visitAddExpr(AddExpr nd) {
				return Jimple.v().newAddExpr(leftHandSideValue, rightHandSideValue);
			}
			@Override
			public Value visitSubExpr(SubExpr nd) {
				return Jimple.v().newSubExpr(leftHandSideValue, rightHandSideValue);
			}
			@Override
			public Value visitMulExpr(MulExpr nd) {
				return Jimple.v().newMulExpr(leftHandSideValue, rightHandSideValue);
			}
			@Override
			public Value visitDivExpr(DivExpr nd) {
				return Jimple.v().newDivExpr(leftHandSideValue, rightHandSideValue);
			}
			@Override
			public Value visitModExpr(ModExpr nd) {
				return Jimple.v().newRemExpr(leftHandSideValue, rightHandSideValue);
			}
